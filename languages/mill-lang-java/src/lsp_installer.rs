...
mod tests {
    ...
    #[test]
    fn test_lsp_name() {
        let installer = JavaLspInstaller;
        assert_eq!(installer.lsp_name(), "jdtls");
    }

    #[test]
    fn test_check_installed() {
        let installer = JavaLspInstaller;
        // This test will pass regardless of whether jdtls is installed
        let result = installer.check_installed();
        assert!(result.is_ok());
    }

    #[test]
    fn test_check_in_path_no_error() {
        // Should not panic even if jdtls is not installed
        let result = check_in_path();
        // Result can be Some or None, we just check it doesn't crash
        let _ = result;
    }

    #[test]
    fn test_check_mason_no_error() {
        // Should not panic even if mason is not installed
        let result = check_mason_installation();
        let _ = result;
    }

    #[test]
    fn test_check_mill_cache_no_error() {
        // Should not panic even if mill cache doesn't exist
        let result = check_mill_cache();
        let _ = result;
    }
}
