...
main() {
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Setup TypeMill (detects languages & installs LSP servers): ${YELLOW}mill setup${NC}"
    echo "  2. Start the server: ${YELLOW}mill start${NC}"
    echo "  3. Check the status: ${YELLOW}mill status${NC}"
    echo ""
    echo "What 'mill setup' does:"
    echo "  • Scans your project to detect languages (TypeScript, Rust, Python)"
    echo "  • Creates .typemill/config.json with LSP configurations"
    echo "  • Auto-downloads missing LSP servers (with your permission)"
    echo "  • Caches LSPs in ~/.mill/lsp/ for reuse across projects"
    echo ""
