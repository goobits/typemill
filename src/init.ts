...
/**
 * Generate a well-commented configuration file for codebuddy
 */
export async function main(): Promise<void> {
  const configPath = resolve(process.cwd(), 'codebuddy.json');

  // Check if config already exists
  if (existsSync(configPath)) {
    console.error('⚠️  Configuration file already exists: codebuddy.json');
    console.error('   To regenerate, please remove the existing file first.');
    process.exit(1);
  }

  console.log('🔧 Initializing codebuddy configuration...\n');
  ...
  console.log('✅ Configuration file created: codebuddy.json\n');
  ...
  console.log('   3. Set CODEBUDDY_CONFIG_PATH environment variable if using MCP');
  console.log('\n💡 Tips:');
  ...
  console.log('   - Use "codebuddy retry" to retry failed servers after installation');
}
...
function generateConfigWithComments(
  ...
  lines.push('{');
  lines.push('  // codebuddy configuration file');
  lines.push('  // Generated by: codebuddy init');
  lines.push('  // Documentation: https://github.com/ktnyt/codebuddy');
  lines.push('  //');
  lines.push(
