...
"#);
    ...
    client.wait_for_ready(Duration::from_secs(10))
        .map_err(|e| anyhow::anyhow!(e.to_string()))?;
    eprintln!("TEST: Server is ready.");

    // 3. Test inspect_code
    eprintln!("TEST: verifying inspect_code...");
    match client.call_tool("inspect_code", json!({
        ...
        Err(e) => eprintln!("ERROR: inspect_code failed (expected if LSP missing): {}", e),
    }

    // 4. Test search_code
    eprintln!("TEST: verifying search_code...");
    match client.call_tool("search_code", json!({
    ...
    match client.call_tool("rename_all", json!({
        "target": {
            "kind": "file",
    ...
    match client.call_tool("relocate", json!({
        "target": {
            "kind": "file",
            "filePath": workspace.path().join("src/main.rs")
        },
    ...
    match client.call_tool("prune", json!({
        "target": {
            "kind": "file",
            "filePath": workspace.path().join("src/utils.ts")
        },
    ...
    match client.call_tool("refactor", json!({
        "action": "extract",
        "params": {
            "kind": "function",
            "filePath": workspace.path().join("src/main.rs"),
    ...
    match client.call_tool("workspace", json!({
        "action": "find_replace",
        "params": {
            "pattern": "Helper",
