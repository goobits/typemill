...
use crate::test_helpers::{
    build_inline_params, extract_content, set_dry_run, try_refactor_tool,
};
...
async fn test_inline_variable_plan_and_apply() {
    ...
    assert_eq!(plan.get("planType").and_then(|v| v.as_str()), Some("inlinePlan"), "Should be InlinePlan");
    ...
    assert_eq!(result.get("status").and_then(|v| v.as_str()), Some("success"), "Inline should succeed");
}
...
async fn test_inline_function_dry_run() {
    let workspace = TestWorkspace::new();
    workspace.create_file("inline_fn.rs", "fn double(x: i32) -> i32 {\n    x * 2\n}\n\npub fn test() -> i32 {\n    double(5)\n}\n");
    let mut client = TestClient::new(workspace.path());
    ...
    assert_eq!(result.get("status").and_then(|v| v.as_str()), Some("success"), "Dry run should succeed");
    assert!(workspace.read_file("inline_fn.rs").contains("fn double(x: i32)"), "File should be unchanged after dry run");
}
...
async fn test_inline_plan_warnings() {
    ...
    assert!(plan.get("warnings").is_some(), "Plan should have warnings field");
}
