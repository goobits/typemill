...
`);
      if (locations.length > 0) {
        const locationResults = locations.map((loc) => {
          const filePath = uriToPath2(loc.uri);
...
${matchDescriptions}`;
  ...
  try {
    ...
    const responseText = dry_run ? `[DRY RUN] Would rename "${symbol_name}" to "${new_name}" across ${changedFileCount} file${changedFileCount === 1 ? "" : "s"}` : `✅ Successfully renamed "${symbol_name}" to "${new_name}" across ${changedFileCount} file${changedFileCount === 1 ? "" : "s"}`;
    return {
      content: [
        {
          type: "text",
...
async function handleRenameSymbolStrict(symbolService, args) {
  ...
  try {
    ...
    const changedFileCount = Object.keys(workspaceEdit.changes).length;
    const responseText = dry_run ? `[DRY RUN] Would rename symbol at ${file_path}:${line}:${character} to "${new_name}" across ${changedFileCount} file${changedFileCount === 1 ? "" : "s"}` : `✅ Successfully renamed symbol at ${file_path}:${line}:${character} to "${new_name}" across ${changedFileCount} file${changedFileCount === 1 ? "" : "s"}`;
    return {
      content: [
        {
          type: "text",
