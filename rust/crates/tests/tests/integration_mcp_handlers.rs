...
async fn test_navigation_find_definition_integration() {
    ...
    workspace.create_file("example.ts", r#"
...
"#);
    ...
    let result = client.call_tool("find_definition", json!({
...
async fn test_document_symbols_integration() {
    let workspace = TestWorkspace::new();
    workspace.create_file("simple.ts", r#"
class TestClass {
    method() {
        return 42;
    }
...
"#);
    ...
    let result = client.call_tool("get_document_symbols", json!({
    ...
    if let Ok(response) = result {
        if let Some(result_obj) = response.get("result") {
            if let Some(tools) = result_obj.get("tools") {
                println!("Available tools: {}", tools.as_array().map_or(0, |arr| arr.len()));
            }
        }
    }
}