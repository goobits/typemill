...
use serde_json::json;
use tests::harness::{create_rename_test_project, TestClient, TestWorkspace};
...
async fn test_rename_symbol_updates_references() {
    ...
    assert!(
        rename_response["error"].is_null(),
        "Rename symbol should not error"
    );
...
"#,
    ...
    let initial_completions = client
        .call_tool(
            "get_completions",
            json!({
    ...
    let updated_completions = client
        .call_tool(
            "get_completions",
            json!({
                "file_path": workspace.absolute_path("src/moduleB.ts").to_str().unwrap(),
    ...
    if let Some(response) = updated_completions {
        if response["error"].is_null() && response["result"]["content"]["items"].is_array() {
            ...
            let has_new_symbol = items
                .iter()
                .any(|item| item["label"].as_str().unwrap_or("").contains("newSymbol"));
...
"#,
    ...
    let initial_diagnostics = client
        .call_tool(
            "get_diagnostics",
            json!({
...
"#,
    ...
    if let Ok(response) = rename_response {
        if response["error"].is_null() {
            ...
            assert!(
                controller_content.contains("DataManager"),
                "Controller should use new name"
            );

            let helper_content = workspace.read_file("src/utils/helper.ts");
            assert!(
                helper_content.contains("DataManager"),
                "Helper should use new name"
            );
    ...
    async fn test_batch_operations() {
        ...
        if let Ok(response) = batch_response {
            if response["error"].is_null() {
                ...
                assert!(
                    workspace.file_exists("src/component1.ts"),
                    "File 1 should be renamed"
                );
                assert!(
                    workspace.file_exists("src/component2.ts"),
                    "File 2 should be renamed"
                );
                assert!(
                    !workspace.file_exists("src/module1.ts"),
                    "Old file 1 should not exist"
                );
                assert!(
                    !workspace.file_exists("src/module2.ts"),
                    "Old file 2 should not exist"
                );
