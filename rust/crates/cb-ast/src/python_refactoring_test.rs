...
mod tests {
    ...
    return total"#;
        ...
        let result = plan_extract_function(source, &range, "calculate_tax", "/tmp/test.py");
    ...
    #[test]
    fn test_python_inline_variable_basic() {
        let source = r#"def test_function():
    multiplier = 2.5
    result = some_value * multiplier + 10
    return result"#;

        let result = plan_inline_variable(source, 1, 10, "/tmp/test.py");
    ...
    #[test]
    fn test_python_extract_variable_basic() {
        let source = r#"def calculate_total(price, tax_rate):
    ...
    fn test_python_language_detection() {
        ...
        let result = plan_extract_variable(source, 0, 4, 0, 9, None, "test.py");
