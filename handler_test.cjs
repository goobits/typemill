...
async function testHandlers() {
  ...
  // Import handlers
  const { handleCreateFile, handleDeleteFile } = await import('./dist/src/mcp/handlers/utility-handlers.js');
  const { handleGetFoldingRanges, handleGetDocumentLinks, handleApplyWorkspaceEdit } = await import('./dist/src/mcp/handlers/advanced-handlers.js');
  const { handleGetSignatureHelp } = await import('./dist/src/mcp/handlers/intelligence-handlers.js');
  ...
  try {
    ...
    try {
      ...
      const success = result.content && result.content[0] && result.content[0].text.includes('Successfully created');
      ...
      testResults.push({ test: 'handleCreateFile', status: (success && fileExists) ? 'PASS' : 'FAIL' });
    } catch (error) {
    ...
    try {
      if (fs.existsSync(createTestFile)) {
        ...
        const success = result.content && result.content[0] && result.content[0].text.includes('Successfully deleted');
        ...
        testResults.push({ test: 'handleDeleteFile', status: (success && fileDeleted) ? 'PASS' : 'FAIL' });
      } else {
    ...
    try {
      ...
      const success = result.content && result.content[0] && result.content[0].text.includes('successfully');
      ...
      testResults.push({ test: 'handleApplyWorkspaceEdit', status: (success && editApplied) ? 'PASS' : 'FAIL' });
    ...
    try {
      ...
      console.log(`   📋 Response preview: ${success ? result.content[0].text.substring(0, 100) + '...' : 'No response'}`);
      
      testResults.push({ test: 'handleGetDocumentLinks', status: success ? (isGracefulDegradation ? 'GRACEFUL' : 'PASS') : 'FAIL' });
    } catch (error) {
