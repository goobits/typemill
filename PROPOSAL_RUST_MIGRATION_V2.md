...
```
Update `rust/Cargo.toml` to declare the workspace members and add `rust/rust-toolchain.toml` to pin the Rust version.
...
### 3.1 Crate Contracts (Expose These APIs)
...
- Provide `pub struct SessionReport` summarizing operations (success/failure counts).
...
bun run test:e2e:rust   # script to run TS suite against Rust server
```
...
- `rust/docs/runbooks/` – Benchmarks, release procedures, operational notes.
...
Version 2 of the migration plan keeps the technical roadmap intact but adds explicit crate contracts so work can be parallelized safely. If each contributor adheres to the exposed APIs above and ships the accompanying acceptance tests, the pieces will integrate cleanly and deliver a Rust backend that meets or exceeds the current TypeScript capabilities.
