...
impl NavigationHandler {
    ...
    fn convert_tool_call_to_plugin_request(&self, tool_call: &ToolCall) -> Result<PluginRequest, crate::ServerError> {
            ...
            _ => {
                ...
                let file_path_str =
                    args.get("file_path")
                        .and_then(|v| v.as_str())
                        .ok_or_else(|| {
                            crate::ServerError::InvalidRequest("Missing file_path parameter".into())
                        })?;
...
impl ToolHandler for NavigationHandler {
    ...
    async fn handle_tool_call(
        ...
        match context.plugin_manager.handle_request(plugin_request).await {
            ...
            Err(err) => {
                Err(crate::ServerError::Internal(format!("Plugin request failed: {}", err)))
            }
        }
    }
}
