...
impl ReferenceUpdater {
    ...
    pub async fn update_references(
        ...
        tracing::info!(
            ...
            "Processing affected files for reference updates (parallel)"
        );

        // OPTIMIZATION: Process files in parallel using JoinSet
        // Each file is processed independently, and results are collected after all tasks complete.
        // This provides significant speedup for large codebases with many affected files.

        // Create shared data structures for parallel tasks
        let plugin_map_arc = Arc::new(plugin_map);
        let old_path_buf = old_path.to_path_buf();
        let new_path_buf = new_path.to_path_buf();
        let project_root_clone = self.project_root.clone();
        let project_files_arc = Arc::new(project_files);
        let merged_rename_info_arc = merged_rename_info.map(Arc::new);
        ...
        for file_path in affected_files {
            ...
            let plugin_map = plugin_map_arc.clone();
            let old_path = old_path_buf.clone();
            let new_path = new_path_buf.clone();
            let project_root = project_root_clone.clone();
            let project_files = project_files_arc.clone();
            let merged_rename_info = merged_rename_info_arc.clone();

            join_set.spawn(async move {
                process_file_for_reference_update(
                    file_path,
                    &plugin_map,
                    &old_path,
                    &new_path,
        ...
        while let Some(res) = join_set.join_next().await {
            match res {
                Ok(Some(edit)) => all_edits.push(edit),
                Ok(None) => {}
                Err(e) => {
                    tracing::error!("Task join error during parallel file processing: {}", e);
                }
            }
        }
...
/// Process a single file for reference updates (used by parallel processing)
///
/// This function is called by each parallel task in `update_references`.
/// It handles all three rename types (package, directory, file) and returns
/// an optional `TextEdit` if changes are needed.
async fn process_file_for_reference_update(
    file_path: PathBuf,
    plugin_map: &HashMap<String, std::sync::Arc<dyn mill_plugin_api::LanguagePlugin>>,
    old_path: &Path,
    new_path: &Path,
    project_root: &Path,
    project_files: &[PathBuf],
    merged_rename_info: Option<&serde_json::Value>,
    is_package_rename: bool,
    is_directory_rename: bool,
) -> Option<TextEdit> {
    tracing::debug!(
        file_path = %file_path.display(),
        "Processing affected file (parallel)"
    );

    // Get plugin for this file extension
    let plugin = file_path
        .extension()
        .and_then(|ext| ext.to_str())
        .and_then(|ext_str| plugin_map.get(ext_str))?;
    ...
    if is_package_rename {
        ...
        let rewrite_result = plugin.rewrite_file_references(
            ...
            &file_path,
            project_root,
            merged_rename_info, // Contains both cargo info AND scope flags
        );
        ...
        if let Some((updated_content, count)) = rewrite_result {
            if count > 0 && updated_content != content {
                ...
                return Some(TextEdit {
                    ...
                    new_text: updated_content,
                    priority: 1,
                    description: format!("Update imports in {} for file rename", file_path.display()),
                });
            }
        }
    }
