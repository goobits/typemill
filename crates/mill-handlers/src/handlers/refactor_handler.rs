...
impl RefactorHandler {
    ...
    /// Create preview response directly from RefactorPlan struct
    /// Optimized to avoid JSON roundtrip for metadata extraction and fix warning serialization
    fn create_preview_response(
        ...
        let file_checksums = plan.checksums();
        ...
        // We still need the JSON for the `changes` field in `WriteResponse`
...
mod tests {
    ...
    #[test]
    fn test_create_preview_response_warnings() {
        use mill_foundation::protocol::{RefactorPlan, ExtractPlan, PlanSummary, PlanWarning, PlanMetadata};
        ...
        assert_eq!(response.diagnostics.len(), 1, "Warnings should be correctly extracted");
