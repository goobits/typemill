...
impl DirectLspAdapter {
    ...
    async fn query_all_servers_for_workspace_symbols(
        ...
        // Collect queries for each unique LSP server
        ...
        for extension in &self.extensions {
            ...
            match self.get_or_create_client(extension).await {
                Ok(client) => {
                    ...
                    let kind_filter_clone = kind_filter;

                    query_futures.push(async move {
                        let extension = extension_clone;
                            ...
                            .await
                        {
                            Ok(response) => {
                                ...
                                if let Value::Array(symbols) = response {
                                    ...
                                    if let Some(target_kind) = kind_filter_clone {
                                    ...
                                    return Some((extension, symbols_found));
                                }
                            }
                            Err(e) => {
        ...
        for result in results {
            if let Some((extension, symbols)) = result {
                queried_servers.push(extension);
                all_symbols.extend(symbols);

                // Check limit during merge (soft limit, might slightly exceed but that's fine)
