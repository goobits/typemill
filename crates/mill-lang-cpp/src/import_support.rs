...
impl ImportMoveSupport for CppImportSupport {
    fn rewrite_imports_for_move(
        ...
        let mut query_cursor = QueryCursor::new();
        let mut edits = vec![];
        let moved_from_dir = moved_from_path.parent().unwrap();
        ...
        for match_ in query_cursor.matches(&query, tree.root_node(), source.as_bytes()) {
            ...
            if import_path_text.starts_with('.') {
                let absolute_import_path = moved_from_dir.join(import_path_text).clean();
                if let Some(new_relative_path) =
                    pathdiff::diff_paths(&absolute_import_path, moved_to_path.parent().unwrap())
                {
