//! Minimal Cargo.toml helpers for legacy consolidation feature
//!
//! NOTE: This module contains legacy Rust-specific helpers for the old consolidation
//! post-processing workflow. New code should use the Unified Refactoring API (rename.plan)
//! which handles consolidation through the language-agnostic plugin system.
...
impl FileService {
    ...
    ///
    /// This is a legacy helper for consolidation post-processing.
    pub(super) async fn merge_cargo_dependencies(
        ...
        info!(
            ...
            "Merging Cargo.toml dependencies (legacy consolidation)"
        );
        ...
        fs::write(target_toml_path, target_doc.to_string())
            .await
            .map_err(|e| {
                ServerError::Internal(format!("Failed to write merged Cargo.toml: {}", e))
            })?;
