...
-   **`WorkspaceManager` Partitioning**: The `WorkspaceManager` no longer uses a simple `workspace_id` as the key. Instead, it uses a composite key `(user_id, workspace_id)`. This change in `../../crates/codebuddy-core/src/workspaces.rs` is the core of the data isolation model.
