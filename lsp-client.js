...
__export(exports_file_scanner, {
  ...
  scanDirectoryForExtensions: () => scanDirectoryForExtensions2,
...
async function scanDirectoryForExtensions2(dirPath, maxDepth = 3, ignoreFilter, debug = false) {
  const extensions = new Set;
  async function scanDirectory(currentPath, currentDepth, relativePath = "") {
    if (currentDepth > maxDepth)
      return;
    try {
      const entries = await readdir2(currentPath);
      if (debug) {
        process.stderr.write(`Scanning directory ${currentPath} (depth: ${currentDepth}), found ${entries.length} entries: ${entries.join(", ")}
`);
      }
      for (const entry of entries) {
        const fullPath = join2(currentPath, entry);
        const entryRelativePath = relativePath ? join2(relativePath, entry) : entry;
...
`);
            ...
            const ext = extname2(entry).toLowerCase().slice(1);
            if (debug) {
...
async function scanProjectFiles(projectPath, languageServers, maxDepth = 3, debug = false) {
  ...
  const extensions = await scanDirectoryForExtensions2(projectPath, maxDepth, ignoreFilter, debug);
...
var import_ignore2, DEFAULT_IGNORE_PATTERNS;
var init_file_scanner = __esm(() => {
  import_ignore2 = __toESM(require_ignore2(), 1);
...
var require_ignore = __commonJS2((exports, module) => {
  function makeArray(subject) {
    return Array.isArray(subject) ? subject : [subject];
  }
  var UNDEFINED = undefined;
...
var capabilityManager2 = new CapabilityManager2;
...
`);
  ...
  async startServer(serverConfig) {
    ...
    const serverKey = JSON.stringify(serverConfig.command);
    capabilityManager2.cacheCapabilities(serverKey, initResult);
    if (initResult && typeof initResult === "object" && "capabilities" in initResult) {
...
`);
      if (!existsSync(process.env.CCLSP_CONFIG_PATH)) {
        throw new Error(`Config file specified in CCLSP_CONFIG_PATH does not exist: ${process.env.CCLSP_CONFIG_PATH}`);
      }
      try {
...
`);
        ...
        throw new Error(`Failed to load config from CCLSP_CONFIG_PATH: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    if (!configPath) {
      throw new Error("configPath is required when CCLSP_CONFIG_PATH environment variable is not set");
    }
    try {
...
`);
    ...
    const fileUri = pathToUri2(filePath);
...
`);
      ...
      process.stderr.write(`[DEBUG getDiagnostics] Unexpected response format, returning empty array
`);
      return [];
    } catch (error) {
...
`);
    const result = await this.protocol.sendRequest(serverState.process, "textDocument/definition", {
      textDocument: { uri: pathToUri2(filePath) },
      position
    });
...
`);
  ...
  async renameSymbol(filePath, position, newName) {
    process.stderr.write(`[DEBUG renameSymbol] Requesting rename for ${filePath} at ${position.line}:${position.character} to "${newName}"
`);
    const serverState = await this.getServer(filePath);
    await serverState.initializationPromise;
    await this.ensureFileOpen(serverState, filePath);
...
`);
    ...
    if (!hasOpenFiles) {
      try {
        const { scanDirectoryForExtensions: scanDirectoryForExtensions3, loadGitignore: loadGitignore2 } = await Promise.resolve().then(() => (init_file_scanner(), exports_file_scanner));
        const gitignore = await loadGitignore2(process.cwd());
        const extensions = await scanDirectoryForExtensions3(process.cwd(), 2, gitignore, false);
